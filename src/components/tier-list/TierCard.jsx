...
export default function TierCard({ tier, index, total }) {
	...
	const placedMonstersByTab = useTierListStore((state) => state.placedMonstersByTab);
	const placeMonster = useTierListStore((state) => state.placeMonster);
	...
	/**
	 * Mappe les IDs stockés vers les objets monstres complets depuis le cache React Query.
	 * @type {import('@/lib/tier-list/store').Monster[]}
	 */
	const monsters = useMemo(() => {
		const placedMonsters = placedMonstersByTab[activeTab] ?? {};
		const monsterIds = placedMonsters[tier.id] ?? [];
		if (monsterIds.length === 0) return [];

		// Récupère les données depuis le cache React Query
		const cached = queryClient.getQueryData(["monsters", activeTab]);
		if (!cached?.results) return [];

		// Mappe les IDs → objets complets
		return monsterIds
			.map((id) => cached.results.find((m) => m.com2us_id === id))
			.filter(Boolean);
	}, [placedMonstersByTab, activeTab, tier.id, queryClient]);
